...
fieldstruct *load_field(char *filename)
{
  ...
  {
    //scan atables for first image that is (at least) 2D
    int ntab = 0;
    do
    {
      tab = pos_to_tab(field->cat, ntab++, 0);
    } while (ntab < field->cat->ntab && tab->naxis < 2);
  }
...

{
  ...
  QPRINTF(OUTPUT, "%s: \"%.20s\" %s %dx%d   %d bits (%s)\n"
          ...
          tab->bitpix > 0 ? (tab->compress_type != COMPRESS_NONE ? "compressed" : "integers") : "floats",
